    ...
    if config_file_exists:
        ...
        for section_name in config.sections():
            ...
            if section_prefix == "part":
                ...
                get_part_config = NumericVersionPartConfiguration

                if 'values' in section_config:
                    gen = (x.strip() for x in section_config['values'].splitlines())
                    section_config['values'] = list(filter(None, gen))
                    get_part_config = ConfiguredVersionPartConfiguration

                part_configs[section_value] = get_part_config(**section_config)
                ...
                if 'serialize' in section_config:
                    section_config_generator = (x.strip() for x in section_config['serialize'].splitlines())
                    section_config['serialize'] = list(filter(None, section_config_generator))
    ...
    if 'new_version' not in defaults and known_args.current_version:
        try:
            if current_version and len(positionals) > 0:
                ...
                logger.info("Values are now: " + kv_string(new_version._values))
